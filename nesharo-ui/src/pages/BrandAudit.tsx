...
const sections = [
  {
    ...
    score: 65,
    status: 'warning',
    summary: 'پیج شما هنوز در ذهن مخاطب جایگاه مشخصی ندارد. تخصص شما در طراحی واضح است، اما «چرا باید شما را انتخاب کنم» در محتوا وجود ندارد.',
    issues: [
  ...
  },
  {
    ...
    score: 78,
    status: 'good',
    summary: 'پیام‌رسانی شما نسبتاً قوی است. هوک‌های کپشن خوب نوشته شده‌اند، اما CTA در اکثر پست‌ها غایب یا ضعیف است.',
    issues: [
  ...
  },
  {
    ...
    score: 70,
    status: 'ok',
    summary: 'پالت رنگی نسبتاً ثابت است اما تایپوگرافی پست‌ها یکدست نیست. فید شما در یک نگاه قابل تشخیص نیست.',
    issues: [
  ...
  },
  {
    ...
    score: 82,
    status: 'good',
    summary: 'محتوای آموزشی شما کیفیت بالایی دارد و مخاطبان را در حوزه تخصصی باور می‌کنند. این قوی‌ترین جنبه برند شماست.',
    issues: [
      'Social Proof (نتایج دانش‌آموزان) در محتوا کم است.',
    ],
    fixes: [
      'هر ماه ۲ پست Case Study از نتایج دانش‌آموزان منتشر کنید.',
    ],
  },
  {
    id: 'consistency',
    ...
    score: 60,
    status: 'bad',
    summary: 'ثبات پایین‌ترین امتیاز شماست. در ۴۵ روز گذشته سه بار فاصله بیش از ۷ روز بدون انتشار داشته‌اید.',
    issues: [
...
const statusConfig = {
  good: { label: 'خوب', color: '#4ade80', bg: 'rgba(74,222,128,0.08)', border: 'rgba(74,222,128,0.2)' },
  ok: { label: 'متوسط', color: '#fb923c', bg: 'rgba(251,146,60,0.08)', border: 'rgba(251,146,60,0.2)' },
  warning: { label: 'نیاز به بهبود', color: '#fb923c', bg: 'rgba(251,146,60,0.08)', border: 'rgba(251,146,60,0.2)' },
  bad: { label: 'ضعیف', color: '#f87171', bg: 'rgba(239,68,68,0.08)', border: 'rgba(239,68,68,0.2)' },
};
          ...
          return (
            <button
              ...
              style={{
                ...
                border: `1px solid ${active === s.id ? c.border : 'var(--border)'}`,
              }}
              onClick={() => setActive(s.id)}
            >
              <div className="font-mono font-bold text-2xl mb-1" style={{ color: c.color }}>{s.score}</div>
      ...
      <div className="grid md:grid-cols-5 gap-6">
          ...
          {sections.map(s => {
            ...
            return (
              <button
                ...
                style={{
                  background: active === s.id ? 'var(--secondary)' : 'var(--card)',
                  border: `1px solid ${active === s.id ? 'rgba(124,58,237,0.3)' : 'var(--border)'}`,
                }}
                onClick={() => setActive(s.id)}
              >
                ...
                <div className="text-left flex flex-col items-end gap-1">
                  <span className="font-mono font-bold text-lg" style={{ color: c.color }}>{s.score}</span>
        ...
        <div className="md:col-span-3 flex flex-col gap-4">
          ...
          >
            ...
            {/* Score bar */}
            <div className="mb-5">
              <div className="h-2 rounded-full mb-1" style={{ background: 'var(--secondary)' }}>
                <div
                  className="h-2 rounded-full transition-all duration-700"
                  style={{
                    width: `${activeSection.score}%`,
                    background: `linear-gradient(90deg, ${cfg.color}, ${activeSection.score > 75 ? '#4ade80' : cfg.color})`,
                  }}
                />
              </div>
            </div>
            ...
            {/* Issues */}
            <div className="mb-4">
              <div className="text-xs font-semibold mb-2 uppercase tracking-wide" style={{ color: 'var(--muted-foreground)' }}>
                مشکلات شناسایی‌شده
              </div>
              <div className="flex flex-col gap-2">
                {activeSection.issues.map((issue, i) => (
                  ...
                  >
                    ...
                    <span style={{ color: 'var(--secondary-foreground)' }}>{issue}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-col gap-2">
                {activeSection.fixes.map((fix, i) => (
                  ...
                  >
                    ...
                    <span style={{ color: 'var(--secondary-foreground)' }}>{fix}</span>
                  </div>
                ))}
              </div>
            </div>
