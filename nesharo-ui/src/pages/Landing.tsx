...
const plans = [
  ...
  },
  {
    ...
    price: '۷۹,۹۰۰',
    period: '/ ماه',
    features: ['تحلیل نامحدود پیج', 'نقشه راه کامل ۳۰/۶۰/۹۰ روزه', 'تقویم محتوای هوشمند', 'AI Coach اختصاصی', 'گزارش هفتگی پیشرفت'],
  ...
  },
  {
    ...
    price: 'سفارشی',
    period: '',
    features: ['مناسب آژانس‌ها', 'مدیریت چند پیج', 'اعضای تیم نامحدود', 'گزارش مشترک و API'],
    cta: 'تماس با ما',
    variant: 'outline',
  },
];
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--background)' }}>
      ...
      <nav
        ...
        style={{ background: 'rgba(7,7,15,0.85)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border)' }}
      >
      ...
      <section className="relative pt-36 pb-24 px-6 text-center hero-gradient overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          >
            ...
            مخصوص مدرس‌ها و تولیدکنندگان محتوای آموزشی اینستاگرام.
          </p>
        ...
        >
          {metrics.map(m => (
            <div key={m.label} className="text-center">
              ...
              <div className="text-sm mt-1" style={{ color: 'var(--muted-foreground)' }}>{m.label}</div>
            </div>
          ))}
        </div>
      </section>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-center">
                    <div className="font-mono font-bold text-lg gradient-text">{t.score}</div>
                    <div className="text-xs" style={{ color: 'var(--muted-foreground)' }}>امتیاز</div>
                  </div>
                </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: 'var(--secondary)', borderBottom: '1px solid var(--border)' }}>
                  <th className="text-right py-4 px-6 font-medium" style={{ color: 'var(--muted-foreground)' }}>ابزار</th>
                  <th className="py-4 px-6 font-medium text-center" style={{ color: 'var(--muted-foreground)' }}>تولید محتوا</th>
                  <th className="py-4 px-6 font-medium text-center" style={{ color: 'var(--muted-foreground)' }}>تحلیل برند</th>
                  <th className="py-4 px-6 font-medium text-center" style={{ color: 'var(--muted-foreground)' }}>نقشه راه</th>
                </tr>
              </thead>
              <tbody>
                {competitors.map((c, i) => (
                  ...
                  >
                    <td className="py-4 px-6 font-medium" style={{ color: c.highlight ? '#c4b5fd' : 'var(--foreground)' }}>
                      {c.name}
                      {c.expensive && <span className="tag tag-orange mr-2">گران‌قیمت</span>}
                      {c.highlight && <span className="tag tag-violet mr-2">شما اینجایید</span>}
                    </td>
                    {[c.content, c.audit, c.roadmap].map((val, j) => (
                      <td key={j} className="py-4 px-6 text-center">
                        {val === true ? (
                          ...
                          <span className="text-xs" style={{ color: '#fb923c' }}>{val}</span>
                        )}
                      </td>
                    ))}
                  </tr>
      ...
      <section className="py-24 px-6" style={{ background: 'var(--muted)' }}>
        <div className="max-w-5xl mx-auto">
          <h2 className="font-display text-3xl font-bold text-center mb-4" style={{ color: 'var(--foreground)' }}>
            قیمت‌گذاری ساده و شفاف
          </h2>
          <p className="text-center mb-12" style={{ color: 'var(--muted-foreground)' }}>
            بدون قرارداد. هر زمان لغو کنید.
          </p>
          <div className="grid md:grid-cols-3 gap-6">
            {plans.map((p, i) => (
              <div
                ...
                style={{
                  background: p.variant === 'primary' ? 'rgba(124,58,237,0.12)' : 'var(--card)',
                  border: p.variant === 'primary' ? '1px solid rgba(124,58,237,0.4)' : '1px solid var(--border)',
                  boxShadow: p.variant === 'primary' ? '0 0 40px rgba(124,58,237,0.12)' : undefined,
                }}
              >
                {p.badge && (
                  <div
                    className="absolute -top-3 right-6 px-3 py-1 rounded-full text-xs font-semibold text-white"
                    style={{ background: 'linear-gradient(135deg, #7c3aed, #4f46e5)' }}
                  >
                    {p.badge}
                  </div>
                ...
                <div className="mb-6">
                  <div className="text-sm mb-2" style={{ color: 'var(--muted-foreground)' }}>{p.name}</div>
                  <div className="flex items-baseline gap-1">
                    <span className="font-display text-3xl font-bold" style={{ color: 'var(--foreground)' }}>
                      {p.price}
                    </span>
                    {p.period && (
                ...
                <ul className="flex flex-col gap-3 mb-8 flex-1">
                  {p.features.map((f, j) => (
                    <li key={j} className="flex items-start gap-2 text-sm" style={{ color: 'var(--secondary-foreground)' }}>
                      <span style={{ color: p.variant === 'primary' ? '#a78bfa' : '#4ade80' }}>✓</span>
                      {f}
                    </li>
                  ))}
                </ul>
                <button
                  ...
                  onClick={onEnterApp}
                  onMouseEnter={e => {
                    if (p.variant !== 'primary') e.currentTarget.style.borderColor = 'rgba(124,58,237,0.4)';
                  }}
                  onMouseLeave={e => {
                    if (p.variant !== 'primary') e.currentTarget.style.borderColor = 'var(--border)';
                  }}
                >
                  {p.cta}
                </button>
      ...
      <section className="py-24 px-6 text-center hero-gradient">
        <div className="max-w-2xl mx-auto">
          <h2 className="font-display text-4xl font-bold mb-6" style={{ color: 'var(--foreground)' }}>
            اولین تحلیل برند شما رایگان است
          </h2>
          <p className="mb-8" style={{ color: 'var(--secondary-foreground)' }}>
            همین الان وارد شوید، پیج اینستاگرام خود را متصل کنید و Brand Score خود را دریافت کنید.
          </p>
          <button
